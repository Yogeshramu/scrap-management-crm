...
exports.updatePurchase = exports.createPurchase = exports.getPurchases = void 0;
const db_1 = require("@nur-afiq/db");
async function generateNextPurchaseId() {
    const currentYear = new Date().getFullYear();
    const count = await db_1.prisma.purchase.count();
...
const createPurchase = async (req, res) => {
    try {
        const { type, supplierId, pickupLocation, logisticsMethod, driverName, agreedPrice, paymentStatus, paymentMethod, vehicleModel, engineIntact, gearboxPresent, catalyticConverter, alloyWheelsCount, vehiclePhoto, lotName, scrapDescription, scrapPhoto, grossTonnageEstimate, transportCompanyId, transportTripFee } = req.body;
...
const updatePurchase = async (req, res) => {
    try {
        const { id } = req.params;
        const { pickupLocation, logisticsMethod, driverName, paymentStatus, paymentMethod, vehicleModel, engineIntact, gearboxPresent, catalyticConverter, alloyWheelsCount, lotName, scrapDescription, grossTonnageEstimate, transportCompanyId, transportTripFee } = req.body;
        const existing = await db_1.prisma.purchase.findUnique({ where: { id } });
        if (!existing)
